...
## Database Notes
- Semantic search currently depends on `supabase/rpc_match_memories.sql`.
- The active search flow assumes **768-dimensional** embeddings.
- When changing schema or RPC behavior, keep the web routes and extension payloads aligned.

## Current Limitations
The project is functional but still has known gaps:
- reloads can create duplicate memories
- site exclusion rules are not complete yet
- extension capture is still too broad
- extension requests are currently pinned to `http://localhost:3000`
- voice-to-page attachment exists by URL but still needs stronger modeling
- user-facing control modes for capture are not complete yet

## Near-Term Priorities
- extension control modes and site exclusions
- reload/dedupe protection
- explicit voice-to-page attachment
- sessions/folders
- richer relation-based organization

## Engineering Notes
- Keep all model calls server-side in `apps/web/app/api`.
- Do not move provider secrets into the extension.
- Treat [AGENTS.md](/home/ken/memento/AGENTS.md) as the repo-wide engineering runbook.

## Status
Memento is an active hackathon-speed prototype. The core loop is in place, but capture controls, exclusions, and data organization still need to be tightened before the product is reliable enough for broader use.
